...
from view_chwRestockStaffDiscreteChwlChwPair as p
    inner join chwdb_scm_chwRestock as r on ( p.chwlID = r.chwlID ) and ( p.chwID = r.supervisedChwID )
...
from view_chwRestockStaffDiscreteChwlChwPair as p
    right outer join chwdb_scm_chwRestock as r on ( p.chwlID = r.chwlID ) and ( p.chwID = r.supervisedChwID )
        ...
        left outer join chwdb_admin_staff as l on r.chwlID            = l.staffID
        left outer join chwdb_admin_staff as w on r.supervisedChwID   = w.staffID
;
...
drop view if exists view_position_cha_supervisor;

create view view_position_cha_supervisor as
...
from view_position_cha as p
    left outer join position_supervisor as ps on ( trim( p. position_id ) like trim( ps.position_id ) ) and ( ps.end_date is null )
        left outer join view_position_chss as s on trim( ps.position_supervisor_id ) like trim( s.position_id )
;
...
DROP procedure IF EXISTS `data_mart_msr_2`;

DELIMITER $$
USE `lastmile_dataportal`$$
CREATE PROCEDURE `data_mart_msr_2` ()
BEGIN
...
CREATE TABLE lastmile_report.mart_view_base_msr_healthdistrict as
SELECT * FROM lastmile_report.view_base_msr_healthdistrict;

DROP TABLE IF EXISTS lastmile_report.mart_view_base_msr_facility;
CREATE TABLE lastmile_report.mart_view_base_msr_facility as
SELECT * FROM lastmile_report.view_base_msr_facility;
...
INSERT INTO lastmile_dataportal.tbl_stored_procedure_log (`proc_name`, `parameters`, `timestamp`) 
VALUES ('END: data_mart_msr_2', 'none', NOW());

...
drop view if exists lastmile_ncha.view_base_position_cha_household_registration;

create view lastmile_ncha.view_base_position_cha_household_registration as
select
      f.county,
      f.health_district,
      ...
      cp.position_id_pk                                     as chss_position_id_pk,
...
from lastmile_ncha.view_position_cha_id_community_id            as  pc
    ...
    left outer join lastmile_ncha.view_community_cha            as  a   on  pc.community_id             = a.community_id
    
    left outer join lastmile_ncha.view_position_cha_person      as  pp  on  pc.position_id_pk           = pp.position_id_pk
        left outer join lastmile_ncha.view_geo_health_facility  as  f   on  pp.health_facility          like f.health_facility
    
    left outer join lastmile_ncha.view_position_cha_supervisor  as  s   on  pc.position_id_pk           = s.position_id_pk
        left outer join lastmile_ncha.view_position_chss_person as  cp  on  s.position_supervisor_id_pk = cp.position_id_pk

order by f.county, f.health_district, f.health_facility, cp.position_id_pk, cp.position_id, pc.position_id_pk, pc.position_id, pc.community_id
;

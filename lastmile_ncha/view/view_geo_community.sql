...
drop view if exists lastmile_ncha.view_geo_community;

create view lastmile_ncha.view_geo_community as
select
      k.county_id,
...
from lastmile_ncha.county as k
    inner join lastmile_ncha.health_district as h on k.county_id = h.county_id
        inner join lastmile_ncha.district as d on h.health_district_id = d.health_district_id
            inner join lastmile_ncha.community as c on d.district_id = c.district_id
                left outer join lastmile_ncha.health_facility as f on trim( c.health_facility_id ) like trim( f.health_facility_id )
;
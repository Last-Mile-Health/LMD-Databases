...
drop view if exists lastmile_ncha.view_history_person_position_id_pk_first;

create view lastmile_ncha.view_history_person_position_id_pk_first as
select
      r.person_id,
      cast( substring_index( group_concat( distinct pr.position_id_pk order by pr.begin_date asc separator ',' ), ',', 1 ) as unsigned ) as position_id_pk,

...
drop view if exists lastmile_ncha.view_history_person_position_id_pk_position_id_first;

create view lastmile_ncha.view_history_person_position_id_pk_position_id_first as
select
      ...
      substring_index( group_concat( distinct trim( pid.position_id ) order by pid.begin_date asc separator ',' ), ',', 1 ) as position_id,
      
      min( pid.begin_date )  as position_id_begin_date,
      
      if( 
          substring_index( group_concat( coalesce( pid.end_date, 'null' ) order by pid.begin_date asc separator ',' ), ',', 1 ) like 'null', 
          null,  
          substring_index( group_concat( pid.end_date order by pid.begin_date asc separator ',' ), ',', 1 )
      )
      as position_id_end_date
           
from lastmile_ncha.view_history_person_position_id_pk_first as pr

...
drop procedure if exists data_mart_snapshot_position_cha_hhr_map;

create procedure data_mart_snapshot_position_cha_hhr_map( in  begin_date        date, 
                                                          in  end_date          date,
                                                          in  unit              varchar( 255 ),
...
begin
    ...
    if  ( not cast( begin_date as date  ) is null ) and 
        ...
        truncate data_mart_snapshot_position_cha_hhr_map;

        set loop_date = begin_date;

        while loop_date <= end_date do

            call snapshot_position_cha_hhr_map( loop_date, position_status );

            insert into data_mart_snapshot_position_cha_hhr_map
            select
            ...
            from faux_cursor_snapshot_position_cha_hhr_map as t;

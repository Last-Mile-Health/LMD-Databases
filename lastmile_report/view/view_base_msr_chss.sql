...
VIEW `lastmile_report`.`view_base_msr_chss` AS
    SELECT 
        `view_base_msr`.`chss_name` AS `chss_name`,
        `view_base_msr`.`chss_id` AS `chss_id`,
    ...
    GROUP BY `view_base_msr`.`chss_id` , `view_base_msr`.`month_reported` , `view_base_msr`.`year_reported`
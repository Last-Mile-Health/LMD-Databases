...
VIEW `lastmile_report`.`view_base_msr_community` AS
    SELECT 
        `view_base_msr`.`community_id_primary` AS `community_id`,
    ...
    GROUP BY `view_base_msr`.`community_id` , `view_base_msr`.`month_reported` , `view_base_msr`.`year_reported`
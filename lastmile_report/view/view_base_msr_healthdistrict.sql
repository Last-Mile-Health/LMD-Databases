...
VIEW `lastmile_report`.`view_base_msr_healthdistrict` AS
    SELECT 
        `view_base_msr`.`health_district` AS `health_district`,
        `view_base_msr`.`health_district_id` AS `health_district_id`,
    ...
    GROUP BY `view_base_msr`.`health_district_id` , `view_base_msr`.`month_reported` , `view_base_msr`.`year_reported`
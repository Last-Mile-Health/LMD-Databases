...
drop view if exists lastmile_report.view_diagnostic_odk_id_invalid_original_total_table_type;

create view lastmile_report.view_diagnostic_odk_id_invalid_original_total_table_type as
...
select
      ...
      sum( if(      replace( d.id_original_value, ' ', '' ) regexp '^[0-9]+$' and 
                not replace( d.id_original_value, ' ', '' ) regexp '^[9]+$', 
                1, 
      ...
      ( sum( replace( d.id_original_value, ' ', '' ) regexp '^[9]+$' )     + 
...
from lastmile_report.view_diagnostic_odk_id as d
    left outer join lastmile_ncha.view_position_id_distinct as p on replace( d.id_original_value, ' ', '' ) like p.position_id and d.id_type like if( p.job_id = 1, 'cha',  if( p.job_id = 3, 'chss', null ) )
